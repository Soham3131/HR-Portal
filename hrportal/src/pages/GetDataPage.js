                ...
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {loginRecords.map(record => {
                            // --- CHANGE: The logic now uses the reliable isTouchDevice flag from the backend ---
                            const isSuspicious = record.isTouchDevice;
                            const deviceType = isSuspicious ? 'Touch Device (Mobile)' : 'Desktop';

                            return (
                                <tr key={record._id} className={record.action === 'Check-in' && isSuspicious ? 'bg-red-50' : ''}>
                                    ...
                                    <td className={`px-6 py-4 font-bold ${isSuspicious ? 'text-red-600' : 'text-green-600'}`}>
                                        {deviceType}
                                    </td>
